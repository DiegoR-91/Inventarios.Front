...
import getSCIUsuariosColumns from "./getSCIUsuariosColumns";
import useGetSCIUsuarios from "./hooks/useGetSCIUsuarios";

const tableColumnsSCIUsuarios = getSCIUsuariosColumns();
...
const Usuarios = () => {
  ...
  const { data, isLoading } = useGetSCIUsuarios(sorting, pagination, rowSize);
  if (isLoading)
    return (
        ...
        <div className="flex w-full gap-4">
          ...
          <div className="flex ml-auto">
            <ButtonCustom
              title="Roles"
              className="bg-greenSecondaryButton text-white rounded-md"
              onClick={() => navigate("/sci/roles")}
            />
          </div>
        </div>
        <div className="flex flex-col pt-5 gap-3">
          <div>
            <span className="text-greenSecondaryButton">Usuarios</span>
          </div>
          <div>
            <PaginatedTable
              columns={tableColumnsSCIUsuarios}
              data={data}
              sorting={sorting}
